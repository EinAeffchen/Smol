...
export default function TagAdder({ ownerType, ownerId, existingTags, onTagAdded }: TagAdderProps) {
    ...
    const handleSelection = async (event: React.SyntheticEvent, newValue: string | TagOption | null) => {
        if (!newValue) return;
        ...
        let tagToAssign = allTags.find(t => t.name.toLowerCase() === finalTagName);
        if (!tagToAssign) {
            try {
